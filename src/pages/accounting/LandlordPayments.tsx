...
const LandlordPayments = () => {
  ...
  const payments = useMemo(() => {
    if (isDemoMode) {
      return generateMockPayments(20).map((p, i) => ({
        ...p,
        tenantName: `Landlord ${Math.floor(i / 3) + 1}`,
        type: 'Rent Payment',
      }));
    }
    return [];
  ...
  return (
    <DashboardLayout title="Landlord Payments">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold">Landlord Payments</h2>
          ...
          <CardContent>
            {isDemoMode && payments.length > 0 ? (
              ...
              <div className="text-center py-10 text-muted-foreground">
                ...
                <p className="text-sm">Landlord payment management functionality will be implemented here</p>
                <p className="text-xs mt-1.5">Record payments to landlords, calculate amounts, and transfer funds</p>
              </div>
            )}
          </CardContent>
        </Card>
